...
impl TryInto<untyped::Term> for Term {
    type Error = ();

    fn try_into(self) -> Result<untyped::Term, Self::Error> {
        match self {
            Self::Variable(x) => Ok(untyped::Term::Variable(x)),
            Self::Abstraction(x, _, t) => Ok(untyped::Term::Abstraction(x, t.try_into()?)),
            Self::Application(t, u) => Ok(untyped::Term::Application(t.try_into()?, u.try_into()?)),
            Self::Product(_, _, _) | Self::Sort(_) => Err(()),
        }
    }
}

impl TryInto<Box<untyped::Term>> for Box<Term> {
    type Error = <Term as TryInto<untyped::Term>>::Error;

    fn try_into(self) -> Result<Box<untyped::Term>, Self::Error> {
